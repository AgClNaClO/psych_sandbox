...
选择的末级元技能 ID 精确展开原子技能；只有展开数量超过配置阈值时才调用独立 embedding
接口筛选，再由模型核对适用依据。默认阈值 24、保留 12 项，不使用 BM25，也不逐层选择。
元技能筛选提示与原子技能完整路径在运行时派生，不修改原始技能树、不读取旧 `.pt` 缓存，
不为原子技能新增 120 字原文摘要。详见 [技能选择说明](../docs/SKILL_SELECTION.md)。
