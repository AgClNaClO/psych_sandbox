...
- 将咨询师升级为 API 驱动的 `plan_react_review_v1`：先规划，再查询技能并观察结果，最后执行回复；
  技能语义选择完全由大模型决定，不使用 BM25 或向量匹配。
...
选择实际技能、策略和最终回复。这里不保存或展示模型的私密逐 token 思维链。
...
2. `run_turn`
   ...
   - 目录按模型选择的元技能 ID 返回原子技能 Observation；不使用 BM25 或向量匹配。
   - 咨询师模型观察结果后，自主选择原子技能、策略并生成结构化决策和回复。
   - 对咨询师输出再次进行安全检查。
