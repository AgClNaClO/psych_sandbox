...
回答经过提前披露检查；首次失败会重试，仍失败则使用符合当前行为信号的安全替代回答。
...
2. `run_turn`
   ...
   - 咨询师生成结构化决策和回复。
   - 对咨询师输出再次进行安全检查。
   - 披露门控区分 retrieved 与 blocked 内容。
   - 内部规划器选择情绪反应、行为、阻抗形式和信任变化。
   - 语言生成器仅根据静态视图、允许记忆和内部信号生成回应。
   - 回答通过提前披露检查后，分别保存状态增量、结构化信号和泄漏审计结果。
...
- 领域模型数值边界。
- 咨询师信息隔离。
- 来访者静态/私有信息隔离、retrieved/blocked 披露门控和提前披露 fallback。
- 泛化触发词抑制、明确话题边界响应和咨询师重复回复修复。
- 两阶段来访者生成、信任因果更新、自然回撤和独立仿真评估。
- 四级风险和输出安全检查。
- 层级技能父子关系、过滤和确定性检索。
- Mock 结构化输出。
- 连续 3-session 运行。
